...
# engine death serial + fighter state
...
# phased ghosts are not targetable during phase
...
# team revive is handled before final victory calculation
...
# angry man revive HP drops by 50 each revival
...
# hero retreat can deal its damage once per retreat
once("if(f.id==='hero'&&f.heroReady&&f.heroPhase==='rush'&&distance(f,e)<=f.radius+e.radius+5){const aim=this.aim(f,e);this.attack(f,e,250*f.scale);f.heroPhase='retreat';f.retreatUntil=this.time+.65/f.scale;f.vx=-aim.x;f.vy=-aim.y;this.effect(f,'돌진!','skill')}",
     "if(f.id==='hero'&&f.heroReady&&f.heroPhase==='rush'&&distance(f,e)<=f.radius+e.radius+5){const aim=this.aim(f,e);this.attack(f,e,250*f.scale);f.heroPhase='retreat';f.retreatHit=false;f.retreatUntil=this.time+.65/f.scale;f.vx=-aim.x;f.vy=-aim.y;this.effect(f,'돌진!','skill')}",'hero retreat flag')

# ghost collision phasing, hero retreat damage, and ghost excluded from normal contact melee
old_collide="""collide(a,b){
 const d=distance(a,b),limit=a.radius+b.radius;if(d>limit+.001)return;
 const n=this.aim(a,b),overlap=Math.max(0,limit-d),wa=a.id==='tree'?0:b.id==='tree'?1:b.radius*b.radius/(a.radius*a.radius+b.radius*b.radius),wb=b.id==='tree'?0:a.id==='tree'?1:1-wa;"""
new_collide="""collide(a,b){
 const d=distance(a,b),limit=a.radius+b.radius;if(d>limit+.001)return;
 if((a.id==='ghost'&&a.ghostPhaseUntil>this.time)||(b.id==='ghost'&&b.ghostPhaseUntil>this.time))return;
 if(a.team!==b.team&&(a.id==='ghost'||b.id==='ghost')){for(const g of [a,b])if(g.id==='ghost'&&g.health>0){g.ghostPhaseUntil=this.time+.3;this.effect(g,'👻 통과 0.3초','skill')}return}
 const n=this.aim(a,b),overlap=Math.max(0,limit-d),wa=a.id==='tree'?0:b.id==='tree'?1:b.radius*b.radius/(a.radius*a.radius+b.radius*b.radius),wb=b.id==='tree'?0:a.id==='tree'?1:1-wa;"""
once(old_collide,new_collide,'collide phase')
once("if(f.id==='boxer'&&f.health>0&&f.stunUntil<=this.time&&this.boxerUppercut(f,e))continue;if(!f.egg&&f.stunUntil<=this.time&&!['archer','mage','tree','ninja','nerd','spider','phoenix','hero','villain','moon','invisible','moai','puffer','police','robot','aladdin','genie','devileye','cursedhand','snowman','angryman','lizardtail'].includes(f.id)&&f.cd<=0){",
     "if(f.id==='boxer'&&f.health>0&&f.stunUntil<=this.time&&this.boxerUppercut(f,e))continue;if(f.id==='hero'&&f.heroReady&&f.heroPhase==='retreat'&&!f.retreatHit&&f.health>0&&f.stunUntil<=this.time){const dealt=this.attack(f,e,250*f.scale);if(dealt>0)f.retreatHit=true;continue}if(!f.egg&&f.stunUntil<=this.time&&!['archer','mage','tree','ninja','nerd','spider','phoenix','hero','villain','moon','invisible','moai','puffer','police','robot','aladdin','genie','devileye','cursedhand','snowman','angryman','lizardtail','ghost'].includes(f.id)&&f.cd<=0){",'hero retreat + ghost melee exclusion')

# Fix historical moon roster text to current 35-second value.
s=s.replace("전투가 45초를 넘기면 궁극기를 준비해.","전투가 35초를 넘기면 궁극기를 준비해.",1)
s=s.replace("· 45초 후 궁극기 ·","· 35초 후 궁극기 ·",1)

# Update boss rules stale moon timing if present.
