...
# CSS for joystick (only visible when explicitly unhidden)
...
# Selection mode state + button status + text
...
# Engine manual movement: ONLY mode=control and left team.
...
# Start control mode with explicit engine option; show joystick only there.
...
# Insert joystick controller after canvas/context declaration.
...
# Feed joystick input only while control mode is active.
...
# Ensure regular mode hint explicitly stays auto-only.
