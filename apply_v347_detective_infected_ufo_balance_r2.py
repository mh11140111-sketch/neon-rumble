...
def rep(a,b,count=1):
    global s
    n=s.count(a)
    if n<count: raise SystemExit(f'missing pattern: {a[:140]!r}')
    s=s.replace(a,b,count)

rep('BATTLE <b>v3.46</b>','BATTLE <b>v3.47</b>')
rep('📒 패치노트 · v3.46</summary><div class="patch-body">','📒 패치노트 · v3.47</summary><div class="patch-body"><div class="patch-version"><h3>v3.47 · 탐정 & 감염 UFO 샌드박스 조정</h3><ul><li>🛸👾 감염 UFO는 샌드박스에서 HP 1250, 파멸의 레이저 주기 10초로 롤백. CHAPTER 4 STAGE 3은 HP 1500 / 8초 유지.</li><li>신규 캐릭터 🕵️‍♂️ 탐정 추가. 🔎 돋보기를 2초마다 던져 피해 75.</li><li>돋보기 적중 시 일반 대상은 2초 동안 크기가 2배가 되며, 커진 대상에게 향하는 투사체는 유도 공격으로 전환. 보스는 크기 증가 면역.</li></ul></div>')

old="{id:'infected_ufo',name:'감염 UFO',icon:'🛸👾',tag:'파멸의 레이저',hp:1500,damage:350,speed:105,cooldown:8,unlock:'infectedUfo',description:'8초마다 아주 두꺼운 파멸의 레이저를 예고 후 발사해. 레이저는 맵 끝까지 관통하고 피해 350을 줘.',detail:'HP 1500 · 파멸의 레이저 350 / 8초 · 발사 전 1초 피격 예고 · 맵 관통'},"
...
rep("function tryUnlockInfectedUfo(){if(infectedUfoUnlocked)return 'already';if(Math.random()>=.05)return 'miss';infectedUfoUnlocked=true;try{localStorage.setItem(INFECTED_UFO_UNLOCK_KEY,'1')}catch{}if($('character-search'))$('character-search').value='';refreshUnlockCards();return 'won'}", "function tryUnlockInfectedUfo(){if(infectedUfoUnlocked)return 'already';if(Math.random()>=.05)return 'miss';infectedUfoUnlocked=true;try{localStorage.setItem(INFECTED_UFO_UNLOCK_KEY,'1')}catch{}if($('character-search'))$('character-search').value='';refreshUnlockCards();return 'won'}\nfunction tryUnlockDetective(){if(detectiveUnlocked)return 'already';if(Math.random()>=.03)return 'miss';detectiveUnlocked=true;try{localStorage.setItem(DETECTIVE_UNLOCK_KEY,'1')}catch{}if($('character-search'))$('character-search').value='';refreshUnlockCards();return 'won'}")
...
rep("infectedUfoSkill(f,e){if(f.id!=='infected_ufo'||f.health<=0||f.stunUntil>this.time||!e||e.health<=0)return;if(!Number.isFinite(f.doomNext))f.doomNext=this.time+8;", "infectedUfoSkill(f,e){if(f.id!=='infected_ufo'||f.health<=0||f.stunUntil>this.time||!e||e.health<=0)return;const doomInterval=f.doomInterval||10;if(!Number.isFinite(f.doomNext))f.doomNext=this.time+doomInterval;")
