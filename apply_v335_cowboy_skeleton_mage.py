...
# Playable Skeleton Mage summons one normal skeleton every 10 seconds. Desert stage 3 keeps its existing stage-specific summon loop only.
old_skill="}else if(f.id==='skeleton_mage'&&f.cd<=1e-9){this.shots.push({x:f.x+a.x*(f.radius+6),y:f.y+a.y*(f.radius+6),vx:a.x,vy:a.y,owner:f.side,team:f.team,target:e.side,kind:'magic',radius:5*f.scale,speed:200*f.scale,damage:65*f.scale,life:5*f.scale,bounces:0});f.cd=1.45/f.scale;f.attack=.18/f.scale;this.effect(f,'💀 마법탄!','skill')}\n}"
new_skill="}else if(f.id==='skeleton_mage'){if(f.cd<=1e-9){this.shots.push({x:f.x+a.x*(f.radius+6),y:f.y+a.y*(f.radius+6),vx:a.x,vy:a.y,owner:f.side,team:f.team,target:e.side,kind:'magic',radius:5*f.scale,speed:200*f.scale,damage:65*f.scale,life:5*f.scale,bounces:0});f.cd=1.45/f.scale;f.attack=.18/f.scale;this.effect(f,'💀 마법탄!','skill')}if(!this.desertWaveMode){if(!Number.isFinite(f.skeletonMageSummonNext))f.skeletonMageSummonNext=this.time+10/f.scale;if(this.time>=f.skeletonMageSummonNext-1e-9){f.skeletonMageSummonNext=this.time+10/f.scale;const ids=['skeleton_gun','skeleton_sword','skeleton_drunk'],id=ids[Math.floor(this.random()*ids.length)],tmp=new Engine('boxer',id,this.random,{mode:'control'}).fighters[1],ang=this.rand(-Math.PI,Math.PI);tmp.side=this.fighters.length;tmp.team=f.team;tmp.summon=true;tmp.ownerSide=f.side;tmp.noSkeletonRevive=true;tmp.revivals=0;tmp.boss=false;tmp.scale=1;tmp.bodyScale=1;tmp.radius=33;tmp.x=clamp(f.x+Math.cos(ang)*82,55,665);tmp.y=clamp(f.y+Math.sin(ang)*82,55,665);tmp.trail=[];this.fighters.push(tmp);this.effect(f,'💀 해골 소환!','skill');this.emit('💀🧙 해골 법사가 '+tmp.name+'을 소환!')}}}\n}"
rep(old_skill,new_skill)

# Stage 3 boss remains HP 1000 even though playable roster base is 700.
old_stage="if(n===3){engine=new Engine('cowboy','skeleton_mage',Math.random,{mode:'control'});engine.fighters[1].noSkeletonRevive=true}"
new_stage="if(n===3){engine=new Engine('cowboy','skeleton_mage',Math.random,{mode:'control'});engine.fighters[1].hp=1000;engine.fighters[1].health=1000;engine.fighters[1].noSkeletonRevive=true}"
rep(old_stage,new_stage)

# Stage 3 clear: bundled skeleton unlock remains guaranteed; Skeleton Mage gets a separate 10% roll every clear until obtained.
old_result="else if(mode==='desert'&&engine.result===0){const skNow=desertStageNo===3?unlockSkeletons():false;$('winner-icon').textContent='🏜️';$('winner').textContent='CHAPTER 2 · STAGE '+desertStageNo+' 클리어!';$('summary').textContent=t+'초 · 사막 돌파 성공'+(skNow?' · 💀 해골들 캐릭터 해금!':'')}"
new_result="else if(mode==='desert'&&engine.result===0){const skNow=desertStageNo===3?unlockSkeletons():false,mageRoll=desertStageNo===3?tryUnlockSkeletonMage():null;$('winner-icon').textContent='🏜️';$('winner').textContent='CHAPTER 2 · STAGE '+desertStageNo+' 클리어!';$('summary').textContent=t+'초 · 사막 돌파 성공'+(skNow?' · 💀 해골들 캐릭터 해금!':'')+(mageRoll==='won'?' · 🎁 10% 보상 성공! 💀🧙 해골 법사 획득!':mageRoll==='miss'?' · 🎲 해골 법사 획득 실패 (10%)':'')}"
rep(old_result,new_result)

p.write_text(s,encoding='utf-8')
print('v3.35 patch applied')
