...
# Zombie Mage sandbox HP nerf only. Stage 3 explicitly resets boss HP to 2000.
...
# Shop + wardrobe controls inserted below patch notes.
...
# Economy state and helpers, inserted before Chapter UI functions.
...
# Button events, placed before stage entry onclick wiring.
anchor="$('stage-entry').onclick=()=>{mode='stage';updateStageUI()};"
insert="$('shop-btn').onclick=()=>toggleEconomyPanel('shop');$('wardrobe-btn').onclick=()=>toggleEconomyPanel('wardrobe');\n"+anchor
rep(anchor,insert)

# Reward coins on each successful stage completion. Stage 3 always fixed 50.
...
# Draw equipped cosmetic above team 0 primary fighters. Exclusive items only show on matching character.
...
print('v3.39 coin/shop/wardrobe patch applied')
